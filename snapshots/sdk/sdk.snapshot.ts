...
function verifyLiveFork(logs: readonly PersistedLog[]): void {
  ...
  const child = logs.find(log => log.header.isSeeded === true)
  if (child === undefined) throw new Error('fork scenario has no seeded child')
  const childEvents = parseSessionLog(child.content)
  const endSeed = childEvents.findLast(event => event.type === 'session/end-seed' && event.data.inherited === true)
  if (endSeed === undefined) throw new Error('fork child has no inherited end-seed marker')
  const inheritedCount = endSeed.seq
...
async function verifyHeaders(
  ...
  for (const [logIndex, log] of ordered.entries()) {
    ...
    const fixtureContent = fixtureContents[logIndex]
    const forkHeaders = logIndex > 0 && log.header.isSeeded === true && fixtureContent !== undefined
      ? normalizedHeaders(fixtureContent, ctx)
      : undefined
    if (headers.length > 0) {
...
describe('TypeScript SDK snapshots over the jsonrpc runtime', () => {
  for (const scenario of sdkScenarios) {
    ...
    scenarioTest(`${mode}s ${scenario.name} through dsh --profile sdk`, async () => {
      ...
      await verifyHeaders(scenario, ordered, actualContext, assertions.dshSdkChild?.agentConfig, expectedContents)
