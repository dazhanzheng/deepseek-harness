...
export interface MaterializeInputs {
  ...
  create?: {
    ...
    /** Child-owned fork closers appended before the descriptor and delegation policies. */
    closers: readonly SessionForkCloser[]
...
export class ContinuableActivationRegistry {
  ...
  private async materializeTracked(
    ...
    const setup = (childCtx: Context, child: Agent): void => {
      ...
      if (create !== undefined) {
        child.session.appendForkClosers(create.closers)
        if (create.seed !== undefined) child.inbox.clear()
