...
import type { CreateSessionOptions, EpochHeader, PrepareSessionOptions, RequestContext, SessionEvent, SessionEventMap, SessionEventType, SessionForkCloser, SessionForkSeed, SessionHeader, SessionId, SessionSeedEventState, SurfaceIntent, SurfaceEventType } from './types.ts'
...
export class Session {
  ...
  appendForkClosers(closers: readonly SessionForkCloser[]): void {
    for (const closer of closers) {
      switch (closer.type) {
        case 'tool/result':
          this.append('tool/result', closer.data, {
            surfaceOp: 'append',
            ...closer.sourceEventSeqs === undefined ? {} : { sourceEventSeqs: [...closer.sourceEventSeqs] },
          })
          break
        case 'step/end':
          this.append('step/end', closer.data)
          break
        case 'turn/end':
          this.append('turn/end', closer.data)
          break
        default:
          assertNever(closer)
      }
