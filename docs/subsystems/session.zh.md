...
`Session.snapshotForFork()` 返回已提交的父前缀及仅属于子级的闭合记录，不发布子会话，也不改变父级。创建方将 `events` 作为构造函数 seed，单独传递 `inheritedEventCount`，并通过 `appendForkClosers()` 追加 `closers`，使继承的待处理工作留在父级；详见 [Session 包](../../packages/core/session/README.zh.md)。
