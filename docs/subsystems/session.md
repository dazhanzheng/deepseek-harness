...
declare class Session {
  ...
  */
   ...
   * @returns the immutable parent prefix, its child-owned closing records, and the exact inherited length.
   */
  snapshotForFork(): SessionForkSeed;
  /**
   * Append a fork seed's child-owned closing records, in order, continuing this
   * session's log. The records close inherited tool calls, steps, and turns
   * without executing or retrying the parent's pending work.
   * @param closers - closing records captured by {@link snapshotForFork}.
   */
  appendForkClosers(closers: readonly SessionForkCloser[]): void;
  /**
...
`Session.snapshotForFork()` returns the committed parent prefix plus the child-only closing records, without publishing a child or changing its parent. Creators pass `events` as the constructor seed, carry `inheritedEventCount` separately, and append `closers` through `appendForkClosers()` so inherited pending work stays with the parent; see the [Session package](../../packages/core/session/README.md).

```ts type-equiv
/** Immutable fork history: the parent's committed prefix plus child-owned closing records. */
interface SessionForkSeed {
  /**
   * The parent's committed prefix, exactly the inherited history the child
   * constructor accepts: contiguous from seq 0 and length `inheritedEventCount`.
   */
  readonly events: readonly SessionEvent[]
  /** Child-owned tool results and step/turn endings that close inherited work. */
  readonly closers: readonly SessionForkCloser[]
  /** Exact parent prefix length; excludes the child-owned closing records. */
  readonly inheritedEventCount: SessionLogOffset
}
```
