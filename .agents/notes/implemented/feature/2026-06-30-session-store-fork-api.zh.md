...
语义上的风险在于 fork 边界。一个合法的用户可见 fork 种子必须连续，并在活跃轮次之外结束。如果在执行过程中 fork，会复制一个未关闭的 `turn/start`、可能还有一个未关闭的 `step/start`，以及可能悬空的工具调用。这违反了执行与提供方 transcript（文本记录）不变式，并且会创建一段误导性的子历史——看起来子会话参与了父会话中一个尚未完成的轮次。已关闭轮次之后的独立上下文和由插件负责写入的纯日志事件是稳定且可 fork 的历史。现有的 [subagent seam](2026-06-21-subagent-capability-seam.zh.md) 有意解决的是另一个问题：工具触发的 subagent fork 通常发生在父轮次仍然打开时，[实时 fork 快照](../architecture/2026-09-05-live-subagent-fork-snapshots.zh.md)仅在子级闭合继承的执行。通用的会话 fork 不应静默裁剪；它应当要么在请求的边界处 fork，要么拒绝请求。
...
**常规会话分支使用两个函数：`snapshot()` 加 `fork()`。** 会话分支消费方会立即创建会话，因此单个带显式 `boundary` 的 `fork()` 让该操作保持直接。subagent 提供方则需要独立的快照数据，因为其既有生命周期所有者负责创建和驱动子级。

**静默裁剪未关闭轮次到最后一个已完成边界。** 常规用户／会话分支不得隐藏所请求边界无效的事实，也不得静默丢弃父轮次尾部。subagent 实时快照会保留该尾部，并仅在子级闭合继承的执行。
