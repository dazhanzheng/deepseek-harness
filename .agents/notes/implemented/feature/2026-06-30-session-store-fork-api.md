...
**Two functions for ordinary session branching: `snapshot()` plus `fork()`.** The session-branching consumer creates a session immediately, so a single `fork()` with an explicit `boundary` keeps that operation direct. Subagent providers separately need snapshot data because their existing lifecycle owners create and drive the child.

**Silently clip open turns to the last completed boundary.** Ordinary user/session branching must not hide an invalid requested boundary or silently drop the parent turn tail. Subagent live snapshots retain that tail and close inherited execution only in the child.
